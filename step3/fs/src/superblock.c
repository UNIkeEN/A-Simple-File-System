...
int init_spb(){
    ...
    spb.s_free_blocks_count = num_disk_block - RESERVE_BLOCK;     //3 for spb, 128 for inode_table
...
int check_spb(){
    ...
    if (num_disk_block > MAX_BLOCK_COUNT) num_disk_block = MAX_BLOCK_COUNT;
    if (num_disk_block <= RESERVE_BLOCK) return -1;
    if ((spb.s_blocks_count + spb.s_free_blocks_count) != num_disk_block) return -1;
    return 0;
}
